...
class SfPdfViewerState extends State<SfPdfViewer> with WidgetsBindingObserver {
  ...
  void _retrieveFormFieldsDetails() {
    for (int i = 0; i < _document!.form.fields.count; i++) {
      ...
      if (field is PdfListBoxField) {
        final PdfListBoxFormFieldHelper helper = PdfListBoxFormFieldHelper(field, pageIndex,
    ...
    if (pdfAnnotation is PdfTextMarkupAnnotation) {
      ...
      switch (textMarkup.textMarkupAnnotationType) {
        case PdfTextMarkupAnnotationType.highlight:
          annotation =
              HighlightAnnotation(textBoundsCollection: _getTextLines(textMarkupRects, pageNumber));
          break;
        case PdfTextMarkupAnnotationType.underline:
          annotation =
              UnderlineAnnotation(textBoundsCollection: _getTextLines(textMarkupRects, pageNumber));
          break;
        ...
        case PdfTextMarkupAnnotationType.squiggly:
          annotation =
              SquigglyAnnotation(textBoundsCollection: _getTextLines(textMarkupRects, pageNumber));
          break;
      }
        ...
        if (signatureField.signature != null) {
          ...
          if (bitmapBytes != null) {
            page.graphics.drawImage(
                ...
                Rect.fromLTWH(helper.bounds.left, helper.bounds.top, helper.bounds.width,
  ...
  Future<void> _loadPdfDocument(bool isPdfChanged, bool isDocumentSaved) async {
    try {
      ...
      final int pageCount =
          await _plugin.initializePdfRenderer(_renderDigitalSignatures() ?? _pdfBytes);
      ...
      _getWidthCancellableOperation =
          CancelableOperation<List<dynamic>?>.fromFuture(_plugin.getPagesWidth());
      ...
      if (errorMessage.contains('Invalid cross reference table') ||
        ...
        if (widget.onDocumentLoadFailed != null) {
          widget.onDocumentLoadFailed!(PdfDocumentLoadFailedDetails('Format Error',
                        ...
                        child: TextFormField(
                          ...
                          decoration: InputDecoration(
                            border: OutlineInputBorder(
                                borderSide: BorderSide(
                              color:
                                  _pdfViewerThemeData!.passwordDialogStyle?.inputFieldBorderColor ??
                                      _themeData!.colorScheme.primary,
                            )),
                            errorBorder: OutlineInputBorder(
  ...
  void _isDocumentLoaded() {
    if (_pdfPages.isNotEmpty &&
      ...
      _pdfScrollableStateKey.currentState!.jumpTo(
          xOffset: xOffset + _layoutChangeOffset.dx, yOffset: yOffset + _layoutChangeOffset.dy);
  ...
  Rect? _getViewportGlobalRect() {
    ...
    if ((widget.pageLayoutMode == PdfPageLayoutMode.single &&
      ...
      viewportGlobalRect =
          Rect.fromLTWH(position.dx, position.dy, containerSize.width, containerSize.height);
    }
              ...
              child: FutureBuilder(
                  ...
                  builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
                    if (snapshot.hasData) {
                      ...
                      final List<Widget> children =
                        ...
                        if (pdfImages[pageIndex] != null) {
                          if (_pageTextExtractor.isEmpty ||
                            ...
                            _pageTextExtractor[index] =
                                _pdfTextExtractor!.extractText(startPageIndex: index);
                          }
                        }
                      ...
                      if (widget.pageLayoutMode == PdfPageLayoutMode.single) {
                        _pageController =
                            PageController(initialPage: _pdfViewerController.pageNumber - 1);
                        ...
                        child = MouseRegion(
                          ...
                          child: SizedBox(
                              height: childSize.height, width: childSize.width, child: child),
                        );
    ...
    Timer(kDoubleTapTimeout, () {
      if (!_isDoubleTapped && _canInvokeOnTap && !isBookmarkViewOpen) {
        if (widget.onTap != null) {
          widget.onTap!(
              PdfGestureDetails(_tappedPageNumber, details.localPosition, _tappedPagePosition));
        }
      }
      for (final FocusNode focusNode in _textBoxFocusNodes) {
    ...
    if (widget.pageLayoutMode == PdfPageLayoutMode.single) {
      ...
      bool canRenderImage = !(_pdfScrollableStateKey
          ...
          !(widget.scrollDirection == PdfScrollDirection.vertical
              ? _pdfScrollableStateKey.currentState?.scrollHeadStateKey.currentState?.isScrolled ??
                  false
          ...
          if (_viewportWidth != 0) {
            ...
            WidgetsBinding.instance.addPostFrameCallback((Duration timeStamp) {
              ...
              _pdfScrollableStateKey.currentState?.forcePixels(
                  Offset(initialOffset.dx * _viewportConstraints.biggest.width, targetOffset));
            });
          }
  ...
  void _updateOffsetOnLayoutChange(
    ...
    if (oldLayoutMode != widget.pageLayoutMode && oldLayoutMode == PdfPageLayoutMode.single) {
      ...
      singleOffset = Offset(
          singleOffset.dx * heightPercentage, (singleOffset.dy - greyArea) * heightPercentage);
        ...
        if (widget.pageLayoutMode == PdfPageLayoutMode.single &&
          ...
          if (_viewportConstraints.maxHeight >
            ...
            _singlePageViewKey.currentState!.jumpOnZoomedDocument(
                _pdfViewerController.pageNumber,
                Offset(
                    xPosition / heightPercentage, (yPosition + greyAreaHeight) / heightPercentage));
          }
        }
      });
    }
  ...
  Size _calculateSize(BoxConstraints constraints, double originalWidth, double originalHeight,
    ...
    Size newSize = constraints
        .constrainSizeAndAttemptToPreserveAspectRatio(Size(originalWidth, originalHeight));
    if ((widget.pageLayoutMode == PdfPageLayoutMode.single ||
            widget.scrollDirection == PdfScrollDirection.horizontal &&
  ...
  void _calculateContextMenuBounds(TextSelectionMenuLocation location) {
    ...
    if (localPosition != Offset.zero) {
      if (localPosition.dy < _viewportGlobalRect!.top + kTextSelectionMenuMargin) {
        localPosition =
            Offset(localPosition.dx, _viewportGlobalRect!.top + kTextSelectionMenuMargin);
      ...
      if (localPosition.dx < _viewportGlobalRect!.left + kTextSelectionMenuMargin) {
        localPosition =
            Offset(_viewportGlobalRect!.left + kTextSelectionMenuMargin, localPosition.dy);
    ...
    if (property == 'jumpToBookmark') {
      ...
      if (widget.pageLayoutMode == PdfPageLayoutMode.continuous) {
        if (_pdfScrollableStateKey.currentState != null) {
          ...
          PageStorage.of(context).writeState(context, _pdfViewerController.zoomLevel,
  ...
  Rect _adjustForRotation(
    ...
    switch (angle) {
      ...
      case PdfPageRotateAngle.rotateAngle180:
        adjustedBounds = Rect.fromLTWH(pageWidth - originalBounds.right,
            pageHeight - originalBounds.bottom, originalBounds.width, originalBounds.height);
        break;
